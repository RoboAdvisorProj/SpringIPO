	...
	<section class="content">
		<div class="row">
			...
			<ul id="imgBox" class="mailbox-attachments clearfix uploadedList" style="list-style:none;"></ul>
					...
					<c:if test="${user.username == noticeVO.writer || sessionScope.SPRING_SECURITY_CONTEXT.authentication.authorities=='[ROLE_ADMIN]'}">
...
$(document).ready(function(){
	...
	$("#postModBtn").on("click", function(){
		formObj.attr("action", "${location}/notice/n_modifyPage");
	...
	$("#postDelBtn").on("click", function(){
		...
		if(arr.length > 0){
			...
			if (confirm("해당 글을 삭제하시겠습니까?") == true){    //확인
				formObj.attr("action", "${location}/notice/n_removePage");
				formObj.submit();
			}else{   //취소
			    return;
			}
	...
	$("#postListBtn ").on("click", function(){
		formObj.attr("method", "get");
		formObj.attr("action", "${location}/notice/n_listPage");
		formObj.submit();
	});
